      ...
      {blocks.map((block, i) => (
        <div key={i} className="group relative rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
          ...
          <div className="absolute -left-10 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button
              onClick={() => i < blocks.length - 1 && moveBlock(i, i + 1)}
              className="rounded p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100"
          ...
          <div className="flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 px-3 py-1.5">
            ...
            {block.type === 'heading' && (
              <span className="text-[10px] text-gray-400">H{block.level}</span>
            )}
            {block.type === 'code' && (
    ...
    case 'text':
      return (
        <textarea
          ...
          className="w-full resize-none bg-transparent px-3 py-2 text-sm outline-none min-h-[80px]"
          rows={4}
        />
      );
    case 'heading':
      return (
        <input
          ...
          onChange={(e) => onChange({ ...block, content: e.target.value })}
          className={cn(
            'w-full bg-transparent px-3 py-2 outline-none font-semibold',
        ...
        <div>
          <div className="flex gap-2 px-3 py-1 border-b border-gray-100 dark:border-gray-700">
            <input
              ...
              className="bg-transparent text-xs text-gray-500 outline-none w-24"
          ...
          <textarea
            value={block.content}
            onChange={(e) => onChange({ ...block, content: e.target.value })}
            className="w-full resize-none bg-[#1e1e2e] text-[#cdd6f4] font-mono text-xs px-3 py-2 outline-none min-h-[100px]"
            rows={6}
          />
        </div>
      );
    case 'callout':
      return (
        <div>
          <select
            value={block.variant}
            onChange={(e) => onChange({ ...block, variant: e.target.value as 'info' | 'warning' | 'danger' | 'success' })}
            className="bg-transparent text-xs text-gray-500 px-3 py-1 outline-none border-b border-gray-100 dark:border-gray-700 w-full"
          >
          ...
          <textarea
            value={block.content}
            onChange={(e) => onChange({ ...block, content: e.target.value })}
            className="w-full resize-none bg-transparent px-3 py-2 text-sm outline-none min-h-[60px]"
            rows={3}
          />
        </div>
      );
      ...
      return (
        <div>
          <textarea
            value={block.content}
            onChange={(e) => onChange({ ...block, content: e.target.value })}
            className="w-full resize-none bg-transparent font-mono text-xs px-3 py-2 outline-none min-h-[100px]"
            rows={6}
          ...
          {block.caption !== undefined && (
            <input
              ...
              className="w-full bg-transparent text-xs text-gray-500 px-3 py-1 outline-none border-t border-gray-100 dark:border-gray-700"
              placeholder="Caption"
            />
          )}
        </div>
  ...
  return (
    <div className="flex items-center justify-center gap-2 py-4">
      ...
      {blockTypes.map((bt) => (
        <button
          key={bt.label}
          onClick={() => onAdd(bt.block)}
          className="rounded-md border border-dashed border-gray-300 dark:border-gray-600 px-3 py-1 text-xs text-gray-500 hover:border-blue-400 hover:text-blue-600 transition-colors"
        >
          + {bt.label}
        </button>
      ))}
